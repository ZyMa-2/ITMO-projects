...
class WeatherData:  # класс для работы с прогнозами для одного города
    ...
    def from_xml(cls, raw_xml_data: str) -> 'WeatherData':
        ...
        for forecast in town:
            children = {child.tag: child.attrib for child in
                        forecast}  # представляем список потомков в виде словаря имя=аттрибуты и откидывая список потомков потому что нам их не должны передавать
...
if __name__ == '__main__':  # условие чтоб запускать только если этот файл не импортируют из другого файла
    ...
    print(
        f"Город '{colorama.Back.CYAN}{wd.name}{colorama.Back.RESET}' ({colorama.Fore.LIGHTBLACK_EX}{wd.index}{colorama.Fore.RESET})")
    for fc in wd:
        ...
        print(
            f"    Осадки: {colorama.Fore.BLUE}{precipitation_texts[fc.phenomena.precipitation]}{colorama.Fore.RESET} ")
        print(
            f"    Атмосферное давление: {colorama.Fore.YELLOW}{fc.pressure.min}{colorama.Fore.RESET}-{colorama.Fore.YELLOW}{fc.pressure.max}{colorama.Fore.RESET} мм.рт.ст")
        print(
            f"    Температура: {colorama.Fore.GREEN}{fc.temperature.min}{colorama.Fore.RESET}-{colorama.Fore.GREEN}{fc.temperature.max}{colorama.Fore.RESET} °C")
        print(
            f"    Ветер: {colorama.Fore.MAGENTA}{fc.wind.min}{colorama.Fore.RESET}-{colorama.Fore.MAGENTA}{fc.wind.max}{colorama.Fore.RESET} м/с на {colorama.Fore.MAGENTA}{direction_texts[fc.wind.direction]}{colorama.Fore.RESET}")
